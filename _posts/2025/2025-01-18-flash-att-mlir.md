...
#### Note
> I write reduction loops that can be parallelized by sequential notation. Just be aware that realization of reduction loops are not written otherwise explicitly mentioned, but they can be, and actually be parallalized in actual implementation.
...
```python
    ...
    return y
```
softmax implementation is straightforward, as given above but in reality, This implementation is often problematic.
...
```python
    ...
    for i in 1 ... M: # Parallel Loop
        for j in 1 ... K_2: # Parallel Loop
            S[i][j] = exp(P[i] - m[i][K_2 - 1]) / d[i][K_2 - 1]
...
### FlashAttention
The key contribution of the FlashAttention v1 paper is that Loop Block 3 and Loop Block 2 can be merged. The idea is very similar to merging two reduction loops in `fastSafeSoftmax`.
