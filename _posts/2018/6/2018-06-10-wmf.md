...
기계 학습, 데이터 마이닝 분야에서 어떠한 문제를 풀기 위해 많은 방법이 제안되었고, 실제로 활용되고 있다. 지금은 deep learning이 엄청난 인기를 끌고 있어서, 딥러닝에서 가장 많이 이용되는, Gradient descent 기반의 methods만을 알고 있는 분도 있을 것이다. 딥러닝에서 이용하는 loss function의 형태는 다른 방식의 optimization을 이용하기 어렵거나 불가능한 형태라 *Gradient Descent*를 사용한다(**정확히 이게 유일한 이유인가는 잘 모르겠습니다.**). 반면, 어떠한 형태의 Objective function은 gradient descent를 이용하는 것보다, 더 효율적이게 최적화하는 방법들이 알려져 있다. 이러한 방법들 중 큰 부분을 차지하는 것은 *convex*한 objective function을 최적화하는 방법인 *convex optimization*이다.  추천 시스템에서 가장 많이 사용되는 알고리즘인, *Weighted Matrix Factorization*의 *objective function*은 *Convex*하다. 즉, **Gradient descent 이외에 다른 Optimization methods를 이용해 모델을 트레이닝 할 수 있다.**

### Solving WMF
*Weighted Matrix Factorization* (이하 *WMF*)의 *objective function*은 다음과 같다.

$$
\sum_{u}\sum_{i}{c_{ui} (p_{ui} - x_{u}^{T}y_{i}^{T} )^2} + \lambda(\sum_u{x_u^Tx_u} +\sum_i{y_i^Ty_i})
$$

> 일반적인 WMF model training에 대해 알고 계신 분은, 바로 Conjugate Gradient를 설명하는 부분으로 넘어가도 괜찮습니다.

$x_u, y_i$는 각각 유저 $u$와 아이템 $i$에 관한 latent factor이며, $p_{ui}$는 유저 $u$가 item $i$와 interaction이 있었는지 아닌지를 나타내는 Boolean indicator, 그리고 $c_{ui}$는 $p_{ui}$에 관한 confidence를 나타낸다. $p_{ui} =1$일 때의 $c_{ui}$가 $p_{ui} = 0$일 때의 $c_{ui}$보다 크다. $\lambda$로 시작하는 term은 regularization term이다.

$Y = [y_0|y_1,|...,|y_m]$이라 정의하고,  $x_u$와 관계가 없는 식을 모두 정리하자. (objective function을 $x_u$에 대해 미분했을 때 사라지는 상수들). 또한, $C^u=\text{diag}(c_{ui}), \text{  } P_u = [p_{u0}, p_{u1,}, ... p_{um}]^T$으로 정의하면, objective function을 다음과 같이 표현할 수 있다.

TODO :begin eqnArray로 고치기.
$$
...
I ignored a few proofs. such that $d_k \bot r_{k+1}$, $r_k \bot r_{k+1}$ and shows only results.
refer the paper I took above.
thus it became
$$
r_{k+1} = r_k - Ad_k
$$
