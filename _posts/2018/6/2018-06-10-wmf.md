...
WMF model의 skeleton은 다음과 같다. 구현의 편의상 **positive interaction의 confidence는 한 값으로 fix했고**`confidence`, loss 함수를 계산할 때에는 positive feedback의 `confidence`를 무시했다.

**note: 약간의 변수명 차이가 있을 수 있습니다.**
1. $P \rightarrow X, X \rightarrow U, Y \rightarrow V$
...
```
`user factor`와 `item factor`을 계산하는 calc_loss 함수를 구현하는 부분이 변경됩니다. (엄청난 속도 차이와 함께..!)
...
def _solve_wmf_cg(X, U, V, confidence, lamb, n_steps=3):
    ...
    for j in range(n_users):
        nonzero_indices = get_nonzero_indices(X, j)
        user_factor = U[j]
        ## calculate residual r = (VTVCuPu - VTCuV.dot
        # ??...
        r = -VTV.dot(user_factor)
        for i in nonzero_indices:
            item_factor = V[i]
            r += (confidence - (confidence - 1) * item_factor.dot(user_factor)) * item_factor

        old_r_square = r.dot(r)
        p = r.copy()
        for step in range(n_steps):
            Ap = VTV.dot(p)
            for i in nonzero_indices:
                Ap += (confidence - 1) * item_factor.dot(p) * item_factor

            # Conjugate Gradient update

            alpha = old_r_square / p.dot(Ap)
            user_factor += alpha * p
            r -= alpha * Ap
            new_r_square = r.dot(r)
