...
### Introduction
이 글은 Implicit Matrix Factorization의 loss function을 Conjugate gradient method를이용해 Optimize하는 방법에 대해 소개한다. 

기계 학습, 데이터 마이닝 분야에서 어떠한 문제를 풀기 위해 많은 방법이 제안되었고, 실제로 활용되고 있다. 지금은 deep learning이 엄청난 인기를 끌고 있어서, 딥러닝에서 가장 많이 이용되는, Gradient descent 기반의 methods만을 알고 있는 분도 많을 p 에서 이용하는 loss function의 형태는 다른 방식의 optimization을 이용하기 어렵거나 불가능한 형태라 *Gradient escent*를 사용한다(**정확히 이게 유일한 이유인가는 잘 모르겠습니다.**). 반면, 어떠한 형태의 Objective function은 gradient descent를 이용하는 것보다, 더 효율적이게 최적화하는 방법들이 알려져 있다. 이러한 방법들 중 큰 부분을 차지하는 것은 *convex*한 objective function을 최적화하는 방법인 *convex optimization*이다.  추천 시스템에서 가장 많이 사용되는 알고리즘인, *Weighted Matrix Factorization의 *objective function*은 *Convex*하다. 즉, **Gradient descent 이외에 다른 Optimization methods를 이용해 모델을 트레이닝 할 수 있다.**

### Solving WMF
*ation* (이하 *WMF*)ALS*의 *objective function*은 다음과 같다.

$$
\sum_{u}\sum_{i}{c_{ui} (p_{ui} - x_{u}^{T}y_{i}^{T} )^2} + \lambda(\sum_u{x_u^Tx_u} +\sum_i{y_i^Ty_i})
$$

> 일반적인 WMF model trainingWeighted Matrix Factoriz에 대해 알고 계신 분은, 바로 Conjugate Gradient를 설명하는 부분으로 넘어가도 괜찮습니다.

$x_u, y_i$는 각각 유저 $u$와 아이템 $i$에 관한 latent factor이며, $p_{ui}$는 유저 $u$가 item $i$와 interaction이 있었는지 아닌지를 나타내는 Boolean indicator, 그리고 $c_{ui}$는 $p_{ui}$에 관한 confidence를 나타낸다. $p_{ui} =1$일 때의 $c_{ui}$가 $p_{ui} = 0$일 때의 $c_{ui}$보다 크다.
$\lambda$로 시작하는 term은 regularization term이다.

Y = [y_0|y_1,|...,|y_m]$이라 정의하고,  $x_u$와 관계가 없는 식을 모두 정리하자. (objective function을 $$x_u$에 대해 미분했을 때 사라지는 m들). 또한, $C^u=\text{diag}(c_{ui}), \text{  } P_u = [p_{u0}, p_{u1,}, ... p_{um}]^T$으로 정의하면, objective function을 다음과 같이 표현할 수 있다.
ODO :begin eqnArray로 고치기.
$$
f(x)_{\text{w.r.t } x} $$\sum_ic_{ui}(p_{ui}-x_u^Ty_i)^2+\lambda x_u^Tx_u
$$
...
$(YC^yY^T+\lambda I)$을 $A$, $YC^uP_u$을 $b$,$P_u^TC^UP_u$을 $c$로 표현한다면, 이 식은 $x_u$에 대한 *Quadratic Equation*, 즉 *convex*함을 알 수 있다.

$$ f(x_u) := x_u^TAx_u + 2bx_u + c$$
$A$는 *Positive definite*이며, *Symmetric*이다. 따라서, 
$$ \frac{\partial}{\partial x_u} f(x_u) = 2Ax_u - 2bx_u = 0 $$
이 equation을 풀면
$$x_u = A^{-1}b = (YC^uY^T+\lambda I)^{-1}{YC^uP_u}$$
...
$x_u$를 업데이트하기 위해, $O(f^2n + f^3)$의 시간 복잡도가 필요하며, 일반적으로, $f << n$이므$O(f^2n)$의 시간 복잡도가 필요하다. $x_u$를 업데이트하면, 같은 방법으로 $y_i$를 업데이트한다. 이 과정을 반복하면 점점 더 최적해에 가까운 $x_u$와 $y_i$를 찾을 수 있다.
...
```
Item factors를 고정시키면, objective는 user factors에 대한 *Least Square Problem*이 된다. 이를 풀어, user factors를 찾는다. 이번에는 user factors를 고정시키고 item factors에 대해 *Least Squares*을 풀어 item factors를 찾는다. 다시 말해서, factors를 alternate해가며 *least squares*를 푼다는 의미에서, 이 알고리즘은 Alternating Least Squares(이하 ALS)라는 이름을 갖게 되었다.

Pseudocode에서 알 수 있듯이, $O(f^2n)$의 계산을 m 번, $O(f^2m)$의 계산을 n번 해주어야 하고, 이게 한 iteration이니까, 이 계산을 k번 더 한다고 하면, 알고리즘의 실제 시간 복잡도는 $O(f^2mnk)$가 된다..! 상당히 느리다.

#### somewhat faster version 1.
위의 naive solution의 문제점은, $YC^uY^T$를 구하는 데에 시간이 너무 많이 걸린다는 점이다. *WMF*를 제안한 [논문](http://yifanhu.net/PUB/cf.pdf)에서, 이 $O(f^2n)$의 시간 복잡도를 줄일 수 있는, 아주 우아한 방법을 제안했는데, 이 아이디어는 다음과 같다.
...
Conjugate Gradient의 유도 및 증명을 저도 잘 이해하고 있지 않다는 생각이 들었습니다. 아마, 제 노트를 보더라도 Conjugate Gradient에 대한 자세한 이해는 힘들 것 같아요. 방법을 자세히 설명한 tutorial의 링크를 남깁니다. 추후 공부를 해서, 개념을 완벽히 이해하게 된다면 그 때 다시 설명을 작성하고 싶어요.

TODO: Conjugate Gradient method 과정 더 자세히 정리하기.
따라서, 여기서는 알고리즘의 목표와, 간단한 과정만 적도록 하겠습니다.

Conjugate Gradient Method는 $f(x) = \frac{1}{2}x^TAx - Ax +b$을 최소화시키는 $x^* \in R^n$ 을 $n$  step만에 construct하는 방법입니다. 이 방법은, $A$가 *Symmetric*하며, *Positive Definite*할 때 적용할 수 있습니다.

Conjugate Gradient Method는 임의의 $x_0$에서 출발해, $n$개의 step 동안 $x_0$에 $n$개의 vector를 더해 $f(x)$를 최소화시키는 답 $x_*$를 만드는 방법입니다.

$A$가 Positive definite하다면, $f(x)$을 최소화시키는 값 $x_*$에 대해 다음을 만족시키는 $n$개의 *linear independent vectors* $D= \{d_0, d_2, .., d_{n-1}\}$가 존재합니다.

$$
x_* - x =\sum_{i=0}^{n-1}\beta_i{d_i}
$$

여기서, $d_i$는 direction을 의미합니다.$\beta_i$는 direction의 크기를 나타냅니다.  따라서, 우리가 적절한 $D$와, 각각의 방향에 맞는 $\beta_i$ for $i \in [0,..,n-1]$ 을 찾으면 정답을 찾을 수 있습니다.

이 $beta_i$와, $D$를 찾기 위해 다음과 같은 notation이 추가적으로 필요합니다.

$$
e_k = x_k - x_* |||
r_k = b - Ax_k = b - A(x_* + e_k) = (b - A*x) - Ae_k = -Ae_{k} = -Ae_{k}
$$

설명::

$$
e_0 = x_0 - x_* = -\sum_{i=0}^{n-1}\beta_id_i,
e_k = x_k - x_* = -\sum_{i=k}^{n-1}\beta_id_i
$$

we should put some more assumptio about $D$ to find it easily.
this is follows.
$$
d_iAd_j = 0
$$
이라는, imposement가 필요합니다.
$$
r_k = -Ae_k = \sum_{i=k}^{n-1}\beta_iAd_i 
$$
이며, 몇가지의 유도 과정을 거치면
TODO : 유도 과정 조금 더 자세히 정리...

$r_{k+1} = r_k - Ad_k$ , and  $d_{k+1} = r_k + \rho d_{k}$

take transpose of it and multiplying $d_{k+1}^TA$ then 
$$
\rho_{k} = \frac{{d_{k}^TAd_{k-1}}}{d_{k-1}^TAd_{k-1}} = \frac{r^T_kr_k}{r_{k-1}^Tr_{k-1}}
$$
(note. $d_k \bot r_{k+1}$, $r_k \bot r_{k+1}$이다.)
$d_k^Tr_k = r_k^Tr_k$ because $d_k^Tr_k=(r_k-\beta_{k-1}v_{k-1})^Tr_k = r_k^Tr_k$
...
x, d, r  = 0, 0, 0
for i in range(n):
	//calculating coefficeints r_new and
	beta = dot(d,r) / dot(d,Ad)
	r_new = r - beta * (A * d)
	rho = dot(r_new,r_new) / dot(r,r)
	//update x, d, r
	x += beta * d
	r = r_new
	d = r - rho * d
...
#### Application to Weighted Matrix Factorization

즉, 이 방법은 임의의 ridge regression 문제를 n번의 iteration을 이용해 풀 수 있다.  n번 반복하면 완전한 $x_*$을 구할 수 있지만,
> $k < n$인 $k$번 반복한다면, $x_*$의 근사값 $x_k$을 구할 수 있다.

ALS를 이용한 wmf solving에서, 매번 user factor $u$와 item factor $v$를 계산하는데,  (1) 이 $u$, $v$는 한번만 이용되고는 버려지며, (2) 또한, 굳이 정확한 $u$와 $v$를 계산할 필요는 없다. 따라서, 정확한 $u$와 $v$를 계산하는 대신, *Conjugate Gradient Method*을 이용해 적당한 근사값 $u_k$와, $v_k$를 계산한 다음 다음 스텝으로 계산하면 다소 정확도는 잃겠지만, Alternating Least Square 알고리즘을 훨씬 빠르게 개선할 수 있습니다.

```python
// approximate ALS algorithm using Conjugate Gradient
for k in [0, num_iteration]
	for u in [0,n]:
		for step in [0, n_step]:
			update x_u using Conjugate Gradient
	for i in [0,m]:
		for step in [0, n_step]:
			update y_i using Conjugate Gradient
```
...
### References
- [Conjugate Gradient Explained](http://cmp.felk.cvut.cz/~alblcene/CG/CG_explained.pdf) by Cenek Albl
- [Lecture on Steepest decesnt and Conjugate Gradient](https://cseweb.ucsd.edu/classes/fa15/cse291-b/ConjugateGradient.pdf) by Prof. Chung-Kuan Cheng
